  ...
  while (1) {
    ...
    for (i = 0; i < ready_fd_count; i++) {
      ...
      if (events[i].data.fd == listen_sockfd) {
        ...
        // Add the new connection to the epoll instance, using edge-triggered and non-blocking mode
        // EpollAdd(epoll_fd, client_sockfd, &event);
        int status = fcntl(client_sockfd, F_GETFL);
        status |= O_NONBLOCK;
        ret = fcntl(client_sockfd, F_SETFL, status);
        ERROR_CHECK(ret, -1, "fcntl");
        event.data.fd = client_sockfd;
        event.events = EPOLLIN | EPOLLET;
        ret = epoll_ctl(epoll_fd, EPOLL_CTL_ADD, client_sockfd, &event);
        ...
        ret = recv(client_sockfd, buffer_out, sizeof(buffer_out), 0);
